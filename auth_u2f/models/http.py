...
class IrHttp(models.AbstractModel):
    ...
    def _authenticate(cls, auth_method='user'):
        res = super(IrHttp, cls)._authenticate(auth_method=auth_method)  # super should raise if 1st factor fails

        if auth_method == 'user':
            cr = cls.pool.cursor()
            try:
                user = api.Environment(cr, request.session.uid, {})['res.users'].browse(request.session.uid)
                if user._u2f_get_device():
                    user.u2f_check_credentials(request.session.u2f_last_challenge, request.session.u2f_token_response)
            finally:
                cr.close()
