...
class AuthU2FController(Home):
    ...
    def u2f_login(self, u2f_token_response=None, redirect=None, **kw):
        user = request.env['res.users'].browse(request.session.uid).sudo(request.session.uid)
        ...
        if request.httprequest.method == 'POST':
            ...
            return http.redirect_with_hash(self._login_redirect(user.id, redirect=redirect))
        else:
